...
// ComputeReportData produces the 64-byte value that the attester embeds in the
// SEV-SNP REPORT_DATA field: SHA-512(SubjectPublicKeyInfo(leaf) || context).
// The verifier calls this to reconstruct the expected value and check it against
// the AMD-signed report, cryptographically tying the evidence to this connection.
func ComputeReportData(context []byte, leaf *x509.Certificate) ([64]byte, error) {
	var out [64]byte
	pub, err := PublicKeyBytes(leaf)
	if err != nil {
		return out, err
