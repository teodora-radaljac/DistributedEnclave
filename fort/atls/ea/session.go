...
func (s *Session) ValidateAuthenticatorWithAttestation(st *tls.ConnectionState, role Role, req *AuthenticatorRequest, authBytes []byte, verifyOpts *x509.VerifyOptions, attPolicy attestation.VerificationPolicy) (*ValidationResult, error) {
	return validateAuthenticator(s, st, role, req, nil, &attPolicy, authBytes, verifyOpts)
}

func (s *Session) ValidateAuthenticatorWithPolicies(st *tls.ConnectionState, role Role, req *AuthenticatorRequest, policy *SpontaneousAuthenticatorPolicy, authBytes []byte, verifyOpts *x509.VerifyOptions, attPolicy attestation.VerificationPolicy) (*ValidationResult, error) {
