...
func ValidateAuthenticatorWithAttestation(st *tls.ConnectionState, role Role, req *AuthenticatorRequest, authBytes []byte, verifyOpts *x509.VerifyOptions, attPolicy attestation.VerificationPolicy) (*ValidationResult, error) {
	return validateAuthenticator(nil, st, role, req, nil, &attPolicy, authBytes, verifyOpts)
}

func ValidateAuthenticatorWithPolicies(st *tls.ConnectionState, role Role, req *AuthenticatorRequest, policy *SpontaneousAuthenticatorPolicy, authBytes []byte, verifyOpts *x509.VerifyOptions, attPolicy attestation.VerificationPolicy) (*ValidationResult, error) {
	return validateAuthenticator(nil, st, role, req, policy, &attPolicy, authBytes, verifyOpts)
}

func validateAuthenticator(session *Session, st *tls.ConnectionState, role Role, req *AuthenticatorRequest, policy *SpontaneousAuthenticatorPolicy, attPolicy *attestation.VerificationPolicy, authBytes []byte, verifyOpts *x509.VerifyOptions) (*ValidationResult, error) {
	if st.Version != tls.VersionTLS13 {
		return nil, ErrNotTLS13
	}
	...
	if present {
		...
		parsed, err := attestation.ParsePayload(extracted)
		if err != nil {
			return nil, err
		}
		var verifierPolicy attestation.VerificationPolicy
		if attPolicy != nil {
			verifierPolicy = *attPolicy
		}
		verified, err := attestation.VerifyPayload(st, attestation.ExporterLabelAttestation, certMsg.Context, leaf, parsed, verifierPolicy)
		if err != nil {
			return nil, err
		}
